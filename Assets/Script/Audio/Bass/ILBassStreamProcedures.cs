using AOT; // Needed for [MonoPInvokeCallback]
using ManagedBass;
using System;
using System.IO;
...
public class ILBassStreamProcedures
{
    ...
    {
        // Pin the stream so it’s accessible from native callbacks
    ...
    public static void SongEndSyncCallback(int handle, int channel, int data, IntPtr user)
    {
        ...
        // Copy delegate locally to avoid race conditions
        var end = instance._songEnd;
        if (end != null)
        {
            // UnityMainThreadCallback ensures it runs safely on Unity's thread
